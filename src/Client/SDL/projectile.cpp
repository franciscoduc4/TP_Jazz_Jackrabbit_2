...
void Projectile::draw_projectile(SDL2pp::Window& window, SDL2pp::Renderer& renderer, std::unique_ptr<SDL2pp::Texture>& projectile, std::vector<BulletDTO>& bullets, PlayerDTO& player, int dir_x_screen, int dir_y_screen, int pos_x, int pos_y) {
	int proj_pixel_x;
	int proj_pixel_w;
	int proj_pixel_y;
	int proj_pixel_h;		
	...
	for (auto b : bullets) {
		...
		if (dir_x_screen != 0) { 
            distance_main_bullet_x = x - pos_x/*player.getX()*/;
            x = dir_x_screen + distance_main_bullet_x;
        }
        if (dir_y_screen != 0) {
            distance_main_bullet_y = y - pos_y/*player.getX()*/;
