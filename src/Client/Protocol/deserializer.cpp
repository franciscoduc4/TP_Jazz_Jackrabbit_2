...
/*
std::map<uint8_t, std::vector<GameInfo>> Deserializer::getGamesList(std::unique_ptr<DTO>& dto) {
    std::map<uint8_t, std::vector<GameInfo>> gamesList;
    auto* gamesListDTO = dynamic_cast<GamesListDTO*>(dto.get());
    for (auto& game : gamesListDTO->getGamesList()) {
        gamesList[game.getGameId()] = GameListInfo(game.getGameName(), game.getMap(),
game.getMaxPlayers(), game.getPlayers(), game.getGameMode());
    }
    return gamesList;
}
*/
...
void Deserializer::receiveSnapshot(std::vector<char>& buffer) {
    ...
    while (i < i_max) {
        uint8_t playerId = static_cast<uint8_t>(buffer[i]);
        i++;
    ...
    while (i < i_max) {
        uint8_t enemyId = static_cast<uint8_t>(buffer[i]);
        i++;
    ...
    while (i < i_max) {
        uint8_t bulletId = static_cast<uint8_t>(buffer[i]);
        i++;
    ...
    // std::unique_ptr<DTO> game = std::make_unique<GameDTO>(players, enemies, bullets, items,
    // weapons, tiles);
