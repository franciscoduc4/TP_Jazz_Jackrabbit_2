...
std::shared_ptr<Enemy> EntityFactory::createFlyer(uint8_t enemyId, Vector<uint32_t> position) {
    return std::make_shared<FlyingEnemy>(gameMap, position, enemyId);
}

std::shared_ptr<Enemy> EntityFactory::createJumper(uint8_t enemyId, Vector<uint32_t> position) {
