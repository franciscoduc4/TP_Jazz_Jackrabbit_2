...
std::unique_ptr<State> LoriSpecialAttackState::move(Character& character, 
Move direction, float time) {
...
std::unique_ptr<State> LoriSpecialAttackState::reload(std::unique_ptr<Weapon> weapon, float time) {
    // No puede recargar mientras realiza un ataque especial
    return nullptr;
}

std::unique_ptr<State> LoriSpecialAttackState::receiveDamage(Character& character, 
