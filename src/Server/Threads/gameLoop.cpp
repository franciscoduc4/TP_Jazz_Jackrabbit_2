...
GameLoopThread::GameLoopThread(std::shared_ptr<Queue<std::unique_ptr<CommandDTO>>> recvQueue,
        ...
        gameMap(gameMap),
        gameId(gameId) {
    std::cout << "[GAME LOOP] GameLoopThread initialized with gameId: " << gameId << std::endl;
}
...
void GameLoopThread::adjustCommandsToProcess(std::chrono::duration<double> processingDuration,
                                             double frameRate) {
    std::cout << "[GAME LOOP] Adjusting commands to process, current count: " << commandsToProcess
              << ", processing duration: " << processingDuration.count()
    ...
    std::cout << "[GAME LOOP] New commands to process count: " << commandsToProcess << std::endl;
}
