...
EnemyDTO::EnemyDTO(uint16_t x, uint16_t y, uint32_t enemy_id, int health, int damage, int speed, EnemyType type):
        x(x), y(y), enemy_id(enemy_id), health(health), damage(damage), speed(speed), type(type) {}
